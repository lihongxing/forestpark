...
<div class="navbar navbar-inverse navbar-static-top" role="navigation" style="position:static;">
    <div class="container-fluid">
        <ul class="nav navbar-nav">
            ...
            {php global $top_nav;}
            {loop $top_nav $nav}
            {if !empty($_W['isfounder']) || empty($_W['setting']['permurls']['sections']) || in_array($nav['name'],
            $_W['setting']['permurls']['sections'])}
            <li
            {if FRAME == $nav['name']} class="active"{/if}><a href="{php echo url('home/welcome/' . $nav['name']);}"><i
                    class="{$nav['append_title']}"></i>{$nav['title']}</a></li>{/if}
            {/loop}
            <li {if $action==
            'emulator'}class="active"{/if}>
        ...
        <ul class="nav navbar-nav navbar-right">
            ...
            <li class="dropdown">
                ...
                <ul class="dropdown-menu">
                    ...
                    {/if}
                    <li><a href="{url 'account/display'}" target="_blank"><i class="fa fa-cogs fa-fw"></i> 管理其它公众号</a>
                    </li>
                    <li><a href="{url 'utility/emulator'}" target="_blank"><i class="fa fa-mobile fa-fw"></i> 模拟测试</a>
                    </li>
                </ul>
            </li>
            <li class="dropdown">
                ...
                <ul class="dropdown-menu">
                    ...
                    {if $_W['role'] == 'founder'}
                    <li class="divider"></li>
                    <li><a href="{url 'system/welcome'}" target="_blank"><i class="fa fa-sitemap fa-fw"></i> 系统选项</a>
                    </li>
                    <li><a href="{url 'system/welcome'}" target="_blank"><i class="fa fa-cloud-download fa-fw"></i> 自动更新</a>
                    </li>
...
<div class="upgrade-tips" id="setmeal-tips">
    <a href="{php echo url('user/edit', array('uid' => $_W['account']['uid']));}" target="_blank">
        您的服务有效期限：{php echo date('Y-m-d', $_W['account']['starttime']);} ~ {php echo date('Y-m-d',
        $_W['account']['endtime']);}.
        ...
        {/if}
    </a><span class="tips-close" style="background:#d03e14;" onclick="check_setmeal_hide();"><i
...
<div class="navbar navbar-inverse navbar-static-top" role="navigation" style="position:static;">
    <div class="container-fluid">
        ...
        <ul class="nav navbar-nav navbar-right">
            <li><a href="javascript:;"><i class="fa fa-user"></i> {$_W['user']['name']} -
                    {$_W['user']['store_name']}</a></li>
            ...
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-3 col-lg-2 big-menu">
                    <div id="search-menu">
                        <input class="form-control input-lg" style="border-radius:0; font-size:14px; height:43px;"
                               type="text" placeholder="输入菜单名称可快速查找">
                    </div>
                    {if $GLOBALS['ext_type'] > 0}
                    <div class="btn-group">
                        <button class="btn {if $GLOBALS['ext_type'] == 1}btn-primary{else}btn-default{/if} ext-type"
                                data-id="1">默认
                        </button>
                        <button class="btn {if $GLOBALS['ext_type'] == 2}btn-primary{else}btn-default{/if} ext-type"
                                data-id="2">系统
                        </button>
                        <button class="btn {if $GLOBALS['ext_type'] == 3}btn-primary{else}btn-default{/if} ext-type"
                                data-id="3">复合
                        </button>
                    </div>
                    {/if}
                    ...
                    <div class="panel panel-default">
                        ...
                        <ul class="list-group collapse in" id="frame-{$k}">
                            ...
                            <li class="list-group-item{$link['active']}"
                                ...
                                style="cursor:pointer; overflow:hidden;" kw="{$link['title']}">
                ...
                <div class="col-xs-12 col-sm-9 col-lg-10">
                    ...
                    <ol class="breadcrumb" style="padding:5px 0;">
                        ...
                        <li><a href="{url 'home/welcome/ext' array('m' => $module['name'])}">{$module_types[$module['type']]['title']}模块
                                - {$module['title']}</a></li>
                    ...
                    <ul class="nav nav-tabs">
                        ...
                        <li><a href="{url 'platform/reply/post' array('m' => $m)}"><i class="fa fa-plus"></i>
                                添加{$module['title']}</a></li>
                        {if !empty($site_urls)}
                        {loop $site_urls $site_url}
                        <li {if $_GPC[
                        'do'] == $site_url['do']} class="active"{/if}><a href="{$site_url['url']}">
                            {$site_url['title']}</a></li>
                        {/loop}
                        {/if}
                    </ul>
