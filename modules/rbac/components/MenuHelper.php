...
class MenuHelper
{
    ...
    {
        $memulistone = Menu::find()
            ->where(array('parent' => null))
            ->orderBy('order')
            ->asArray()
            ->all();
        $menulist = array();
        foreach($memulistone as $key => $item){
            array_push($menulist, $item);
            static::levelMenu($menulist, $key);
        }
        return $menulist;
    }
     ...
     * @param $key
     */
    private static function levelMenu(&$menulist, $key)
    {
        $menulisttmp = $menulist[$key];
        if(!empty($menulisttmp)){
            $memulistlevel = Menu::find()
                ->where(array('parent' => $menulisttmp['id']))
                ->orderBy('order')
                ->asArray()
                ->all();
            if(!empty($memulistlevel)){
                $menulist[$key]['child'] = &$memulistlevel;
                foreach($memulistlevel as $key => $item){
                    static::levelMenu($memulistlevel, $key);
                }
            }
        }
    }
