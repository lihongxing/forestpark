...
class UploadHelper {//类定义开始
    ...
    public function upload($savePath ='') {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                // 自动检查附件并限制文件名长度(文件名太长会出bug)
                if($this->autoCheck) {
                    if(!$this->check($file)&&strlen($file['name']) < 64)
                        return false;
                }
                ...
                if(function_exists($this->hashType)) {
                    //$fun =  $this->hashType;
                    $file['hash'] = md5_file($this->autoCharset($file['savepath'] . $file['savename'], 'utf-8', 'gbk'));
                }
                //上传成功后保存文件信息，供其他地方调用
    ...
    public function getUploadFileInfo() {
        $info = array(
            'name' => $this->uploadFileInfo[0]['name'],
            'ext' => $this->uploadFileInfo[0]['extension'],
            'filename' => $this->uploadFileInfo[0]['savename'],
            'attachment' => substr($this->uploadFileInfo[0]['savepath'],1,strlen($this->uploadFileInfo[0]['savepath'])).$this->uploadFileInfo[0]['savename'],
            'url' => yii::$app->params['siteurl'].substr($this->uploadFileInfo[0]['savepath'],1,strlen($this->uploadFileInfo[0]['savepath'])-1).$this->uploadFileInfo[0]['savename'],
            'is_image' => substr($this->uploadFileInfo[0]['type'], 0, 5) == 'image' ? 1 : 2,
            'filesize' => $this->uploadFileInfo[0]['size']
        );
        return $info;
    }
