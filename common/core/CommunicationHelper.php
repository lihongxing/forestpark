...
class CommunicationHelper
{
    public static function ihttp_request($url, $post = '', $extra = array(), $timeout = 60)
    {
        ...
        if (GlobalHelper::strexists($url, 'https://') && !extension_loaded('openssl')) {
            if (!extension_loaded("openssl")) {
                GlobalHelper::message('请开启您PHP环境的openssl');
            }
        }
        if (function_exists('curl_init') && function_exists('curl_exec')) {
            $ch = curl_init();
            if (GlobalHelper::ver_compare(phpversion(), '5.6') >= 0) {
            ...
            curl_close($ch);
            if ($errno || empty($data)) {
                return GlobalHelper::error(1, $error);
            } else {
