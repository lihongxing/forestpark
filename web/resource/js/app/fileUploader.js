define(["jquery", "underscore", "webuploader", "jquery.jplayer", "bootstrap"], function (a, b, c) {
	var d = {
		...
		init : function (b, c) {
			var d = this;
			return d.options = a.extend({}, d.defaultoptions, c),
		...
		initRemote : function () {
			...
			b.modalobj.find(".btn-primary").click(function () {
				...
				c.length > 0 && a.getJSON("/upload/fetch.html?type=image&module="+module, {
					url : c
				}, function (a) {
		...
		initLocal : function () {
			...
			a.modalobj.find("#history_image")[0] || a.modalobj.find(".modal-body").append(this.buildHtml().localDialog);
		...
		localPage : function (c) {
			...
			module = $("#module").val();
			return a.getJSON("/upload/local.html?type=image&module="+module, {
				page : c,
				year : e,
				month : f
			}, function (c) {
				c = c.message,
				g.find(".history-content").css("text-align", "center").html('<i class="fa fa-spinner fa-pulse fa-5x"></i>'),
				g.find("#image-list-pager").html(""),
				b.isEmpty(c.items) ? g.find(".history-content").css("text-align", "center").html('<i class="fa fa-info-circle"></i> 暂无数据') : (g.data("attachment", c.items), g.find(".history-content").empty(), g.find(".history-content").html(b.template(d.buildHtml().localDialogLi)(c)), g.find("#image-list-pager").html(c.page), g.find(".pagination a").click(function () {
						...
						var b = a(this),
						c = a(this).data("id");
						return c ? (a.post("/upload/delete.html", {
								id : c
							}, function (a) {
			...
			g.find(".btn-primary").unbind("click").click(function () {
				...
				g.find(".img-item-selected").each(function () {
					b.push(d.modalobj.find("#history_image").data("attachment")[a(this).attr("attachid")]),
		...
		selectImage : function (b) {
			...
			c.options.multiple || c.modalobj.find("#history_image").find(".btn-primary").trigger("click")
		},
		initLocalAudio : function () {
			var a = this;
			a.modalobj.find("#li_history_audio").removeClass("hide"),
			a.modalobj.find("#history_audio")[0] || a.modalobj.find(".modal-body").append(this.buildHtml().localAudioDialog);
		...
		localAudioPage : function (c) {
			...
			module = $("#module").val();
			return a.getJSON("/upload/local.html?type=audio&pagesize=5&module="+module, {
				page : c
			}, function (c) {
				c = c.message,
				e.find(".history-content").html('<i class="fa fa-spinner fa-pulse"></i>'),
				b.isEmpty(c.items) || (e.data("attachment", c.items), e.find(".history-content").empty(), e.find(".history-content").html(b.template(d.buildHtml().localAudioDialogLi)(c)), e.find("#image-list-pager").html(c.page), e.find(".pagination a").click(function () {
						d.localAudioPage(a(this).attr("page"))
					}), e.find(".js-btn-select").click(function (b) {
						a(b.target).toggleClass("btn-primary"),
						d.options.multiple || d.modalobj.find("#history_audio").find(".modal-footer .btn-primary").trigger("click")
					}), d.playAudio())
			}),
		...
		initImageUploader : function () {
			function b(b) {
				...
				"invalid" === b.getStatus() ? j(b.statusText) : (g.text("预览中"), d.makeThumb(b, function (b, c) {
						if (b)
							return void g.text("不能预览");
						var d = a('<img src="' + c + '">');
						g.empty().append(d)
					}, thumbnailWidth, thumbnailHeight), percentages[b.id] = [b.size, 0], b.rotation = 0),
				b.on("statuschange", function (a, d) {
					"progress" === d ? f.hide().width(0) : "queued" === d && (c.off("mouseenter mouseleave"), e.remove()),
					"error" === a || "invalid" === a ? (j(b.statusText), percentages[b.id][1] = 1) : "interrupt" === a ? j("interrupt") : "queued" === a ? percentages[b.id][1] = 0 : "progress" === a && (h.remove(), f.css("display", "block")),
			...
			function g() {
				var a,
				b = "";
				"ready" === state ? b = "选中" + fileCount + "张图片，共" + c.formatSize(fileSize) + "。" : "confirm" === state ? (a = d.getStats(), a.uploadFailNum && (b = "已上传" + a.successNum + "张图片," + a.uploadFailNum + '张图片上传失败，<a class="retry" href="#">重新上传</a>失败图片或<a class="ignore" href="#">忽略</a>')) : (a = d.getStats(), b = "共" + fileCount + "张（" + c.formatSize(fileSize) + "），已上传" + a.successNum + "张", a.uploadFailNum && (b += "，失败" + a.uploadFailNum + "张")),
				m.html(b)
			}
			function h(a) {
				var b;
				if (a !== state) {
					...
					case "pedding":
						o.removeClass("element-invisible"),
						k.hide(),
						d.refresh();
						break;
					case "ready":
						o.addClass("element-invisible"),
						k.show(),
						d.refresh();
						break;
					case "uploading":
					...
					case "paused":
						...
						break;
					case "confirm":
						if (p.hide(), n.text("确定使用").addClass("disabled"), b = d.getStats(), b.successNum && !b.uploadFailNum)
			...
			var q = {
				...
				server : "/upload/upload.html?type=image&module="+module,
				chunked : !1,
				compress : !1,
				accept : {
					title : "Images",
		...
		initAudioUploader : function () {
			function b(b) {
				...
				"invalid" === b.getStatus() ? l(b.statusText) : (h.text(c.formatSize(b.size) + " kb"), percentages[b.id] = [b.size, 0], b.rotation = 0),
				b.on("statuschange", function (a, c) {
					"progress" === c ? g.hide().width(0) : "queued" === c && (e.off("mouseenter mouseleave"), f.remove()),
					"error" === a || "invalid" === a ? (l(b.statusText), percentages[b.id][1] = 1) : "interrupt" === a ? l("interrupt") : "queued" === a ? percentages[b.id][1] = 0 : "progress" === a && j.remove(),
			...
			function g() {
				var a,
				b = "";
				"ready" === state ? b = "选中" + fileCount + "个音频，共" + c.formatSize(fileSize) + "。" : "confirm" === state ? (a = d.getStats(), a.uploadFailNum && (b = "已上传" + a.successNum + "个音频," + a.uploadFailNum + '个音频上传失败，<a class="retry" href="#">重新上传</a>失败音频文件或<a class="ignore" href="#">忽略</a>')) : (a = d.getStats(), b = "共" + fileCount + "张（" + c.formatSize(fileSize) + "），已上传" + a.successNum + "个", a.uploadFailNum && (b += "，失败" + a.uploadFailNum + "个")),
				m.html(b)
			}
			function h(a) {
				var b;
			...
			var q = {
				...
				paste : "#uploader",
				swf : "/resource/componets/webuploader/Uploader.swf",
				server : "/upload/upload.html?&type=audio",
				chunked : !1,
				compress : !1,
				accept : {
					title : "Audios",
			...
			q = a.extend({}, q, i.options.uploader),
			a("#dndArea p").html("最大支持 " + c.formatSize(q.fileSizeLimit) + " MB 以内的语音 (" + q.accept.extensions + " 格式)"),
			q.pick.multiple = i.options.multiple,
			d = c.create(q),
			d.uploadedFiles = [],
			d.addButton({
