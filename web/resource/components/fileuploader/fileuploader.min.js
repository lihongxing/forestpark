!function (a, b) {
    "function" == typeof define && define.amd ? define(["jquery", "underscore", "webuploader", "jquery.jplayer", "bootstrap", "filestyle"], function (c, d, e) {
    ...
    var d = {
        defaultoptions: {
            ...
            var d = this;
            if (d.options = a.extend({}, d.defaultoptions, c), d.options.callback = b, this.options.isWechat) {
                if (c.account_error)return util.message("公众号号没有上传素材的权限", "", "info"), !1
            } else this.options.global ? this.options.global = "global" : this.options.global = "", document.cookie = "__fileupload_type=" + escape(this.options.type), document.cookie = "__fileupload_dest_dir=" + escape(this.options.dest_dir), document.cookie = "__fileupload_global=" + escape(this.options.global);
            return a("#modal-webuploader").remove(), 0 == a("#modal-webuploader").length && a(document.body).append(d.buildHtml().mainDialog), d.modalobj = a("#modal-webuploader"), d.modalobj.modal("show"), d.modalobj.on("shown.bs.modal", function () {
                if (!a(this).data("init")) {
                    switch (d.options.type) {
                        case"image":
                        case"thumb":
                            d.options.isWechat || d.initRemote(), d.initLocal();
                            break;
                        case"audio":
                            d.initLocalAudio();
                            break;
                        case"voice":
                            ...
                            break;
                        case"video":
                            d.options.isWechat || d.initVideoRemote(), d.options.allowUploadVideo && d.initLocalVideo()
                    }
            ...
            function e(e) {
                var f = a('<li id="' + e.id + '"><p class="title"' + ("audio" == b || "voice" == b ? 'style="top:40px;"' : "") + ">" + e.name + '</p><p class="imgWrap"' + ("audio" == b || "voice" == b ? 'style="top:30px;"' : "") + "></p></li>"), g = a('<div class="file-panel"><span class="cancel">删除</span></div>').appendTo(f), h = f.find("p.progress span"), i = f.find("p.imgWrap"), j = a('<p class="error"></p>'), k = function (a) {
                    switch (a) {
                        case"exceed_size":
                            text = "文件大小超出";
                            break;
                ...
                "invalid" === e.getStatus() ? k(e.statusText) : ("image" == b ? (i.text("预览中"), d.makeThumb(e, function (b, c) {
                    if (b)return void i.text("不能预览");
                    var d = a('<img src="' + c + '">');
                    i.empty().append(d)
                }, thumbnailWidth, thumbnailHeight)) : i.text(c.formatSize(e.size) + " kb"), percentages[e.id] = [e.size, 0], e.rotation = 0), e.on("statuschange", function (a, c) {
                    "progress" === c ? h.hide().width(0) : "queued" === c && (f.off("mouseenter mouseleave"), g.remove()), "error" === a || "invalid" === a ? (k(e.statusText), percentages[e.id][1] = 1) : "interrupt" === a ? k("interrupt") : "queued" === a ? percentages[e.id][1] = 0 : "progress" === a && (j.remove(), "image" == b && h.css("display", "block")), f.removeClass("state-" + c).addClass("state-" + a)
                }), f.on("mouseenter", function () {
                    g.stop().animate({height: 30})
                }), f.on("mouseleave", function () {
                    g.stop().animate({height: 0})
                }), g.on("click", "span", function () {
                ...
                }), p.options.multiple && r.find(".fileinput-button").show(), f.insertBefore(r.find(".fileinput-button"))
            }

            function f(b) {
                var c = a("#" + b.id);
                delete percentages[b.id], g(), c.off().find(".file-panel").off().end().remove()
            }

            function g() {
                var b, c = 0, d = 0, e = w.children();
                a.each(percentages, function (a, b) {
                    d += b[0], c += b[0] * b[1]
                }), b = d ? c / d : 0, e.eq(0).text(Math.round(100 * b) + "%"), e.eq(1).css("width", Math.round(100 * b) + "%"), h()
            }
            ...
            function h() {
                ...
                if ("ready" === state) {
                    if (p.options.isWechat) {
                        if ("" == p.options.mode)var e = p.modalobj.find(".nav-pills li.active").attr("data-mode"); else var e = p.options.mode;
            ...
            function i(b) {
                ...
                if (b !== state) {
                    switch (u.removeClass("state-" + state), u.addClass("state-" + b), state = b, state) {
                        case"pedding":
                            v.removeClass("element-invisible"), r.hide(), d.refresh();
                            break;
                        case"ready":
                            v.addClass("element-invisible"), p.options.isWechat && "video" == p.options.type && a("#upload form").removeClass("hide"), r.show(), d.refresh();
                            break;
                        case"uploading":
                            w.show(), u.text("暂停上传");
                            break;
                        ...
                        case"finish":
                            if (u.removeClass("disabled"), c = d.getStats(), c.successNum) {
                                if (d.uploadedFiles.length > 0)return p.finish(d.uploadedFiles), void d.resetUploader()
            ...
            switch (b) {
                case"image":
                    j = "图片", typeUnit = "张", k = {
                        ...
                        mimeTypes: "image/*"
                    }, l = 30, m = 5242880, n = l * m, o = p.options.isWechat ? {
                        quality: 80,
                        preserveHeaders: !0,
                ...
                case"voice":
                    j = "音频", typeUnit = "个", k = {
                        ...
                        mimeTypes: "audio/*"
                    }, l = 30, m = 6291456, n = l * m, o = !1, p.options.isWechat && (l = 5, "temp" == p.options.mode ? (k.extensions = "mp3", m = 2097152, n = 10485760) : (m = 5242880, n = 26214400));
                    break;
                case"video":
                    j = "视频", typeUnit = "个", k = {
                        title: "Video",
                        ...
                        mimeTypes: "video/*"
                    }, l = 30, m = 20971520, n = l * m, o = !1, p.options.isWechat && (l = 5, "temp" == p.options.mode ? (k.extensions = "mp4", m = 10485760, n = 52428800) : (m = 20971520, n = 104857600))
            }
            p.options.isWechat ? (p.options.flag = 0, p.modalobj.find("#li_upload_perm a").html("上传永久" + j), p.modalobj.find("#li_upload_temp a").html("上传临时" + j + "(保留3天)")) : p.modalobj.find("#li_upload a").html("上传" + j), p.modalobj.find(".modal-body").append(this.buildHtml().uploaderDialog);
            var q = a("#uploader"), r = a('<ul class="filelist"><li class="fileinput-button js-add-image" id="filePicker2" style="display:none;"> <a href="javascript:;" class="fileinput-button-icon">+</a></li></ul>').appendTo(q.find(".queueList")), s = q.find(".statusBar"), t = s.find(".info"), u = q.find(".uploadBtn"), v = q.find(".placeholder"), w = s.find(".progress").hide();
            q.find(".btn-primary");
            ...
            var x = {
                pick: {id: "#filePicker", label: "点击选择" + j, multiple: !0},
                dnd: "#dndArea",
                paste: "#uploader",
                swf: "./resource/componets/webuploader/Uploader.swf",
                server: p.options.isWechat ? "/upload/upload.html?c=utility&module="+module+"&a=wechat_file&do=upload" : "/upload/upload.html?c=utility&module="+module+"&a=file&do=upload&type=" + b,
                compress: o,
                accept: k,
                fileNumLimit: l,
                fileSizeLimit: n,
            ...
            x = a.extend({}, x, p.options.uploader), x.pick.multiple = p.options.multiple, x.isWechat = p.options.isWechat, x.type = p.options.type, "audio" == b || "voice" == b ? p.options.isWechat ? a("#dndArea p").html("临时语音只支持amr/mp3格式,大小不超过为2M,长度不超过60秒<br>永久语音只支持mp3/wma/wav/amr格式,大小不超过为5M,长度不超过60秒") : a("#dndArea p").html("最大支持 " + c.formatSize(x.fileSingleSizeLimit) + " MB 以内的语音 (" + x.accept.extensions + " 格式)") : "video" == b && (p.options.isWechat ? a("#dndArea p").html("临时视频只支持mp4格式,大小不超过为10M<br>永久视频只支持rm/rmvb/wmv/avi/mpg/mpeg/mp4格式,大小不超过为20M") : a("#dndArea p").html("最大支持 " + c.formatSize(x.fileSingleSizeLimit) + " MB 以内的视频 (" + x.accept.extensions + " 格式)")), p.options.fileSizeLimit && (x.fileSizeLimit = p.options.fileSizeLimit), console.dir(x), d = c.create(x), d.uploadedFiles = [], d.addButton({
                id: "#filePicker2",
                label: "+",
                multiple: p.options.multiple
            }), k = 0, d.resetUploader = function () {
                if (fileCount = 0, fileSize = 0, k = 0, d.uploadedFiles = [], a.each(d.getFiles(), function (a, b) {
                        f(b)
                    }), p.options.isWechat) {
                    p.options.video && (a("#upload :text[name='title']").val(""), a("#upload :text[name='introduction']").val("")), g(), d.reset(), d.refresh(), a("#dndArea").removeClass("element-invisible"), a("#uploader").find(".filelist").empty(), a("#filePicker").find(".webuploader-pick").next().css({left: "190px"});
                    var b = a("#uploader").find(".statusBar");
                    b.find(".info").empty(), b.find(".accept").empty(), b.hide()
                } else d.refresh(), d.reset(), u.removeClass("disabled"), i("pedding")
            }, d.onUploadProgress = function (b, c) {
                var d = a("#" + b.id), e = d.find(".progress span");
                e.css("width", 100 * c + "%"), percentages[b.id][1] = c, fileid = b.id, g()
            }, d.onFileQueued = function (a) {
                fileCount++, fileSize += a.size, 1 === fileCount && (v.addClass("element-invisible"), s.show()), e(a), i("ready"), g()
            }, d.onFileDequeued = function (a) {
                fileCount--, fileSize -= a.size, fileCount || i("pedding"), f(a), g()
            }, d.on("all", function (a) {
                switch (a) {
                    case"uploadFinished":
                        i("confirm");
                ...
                if ("pedding" != state && d.options.isWechat && "video" == d.options.type) {
                    var b = a('#upload :text[name="title"]').val(), c = a('#upload textarea[name="introduction"]').val();
                    if (!b)return util.message("视频标题不能为空"), !1;
                    if (!c)return util.message("视频描述不能为空"), !1;
                    d.option("formData", {title: b, introduction: c})
                }
                "ready" === state ? d.upload() : "paused" === state ? d.upload() : "uploading" === state && d.stop()
            }), t.on("click", ".retry", function () {
                d.retry()
            }), t.on("click", ".ignore", function () {
            }), u.addClass("state-" + state), g()
            ...
            b.modalobj.find("#li_network").removeClass("hide"), b.modalobj.find(".modal-body").append(b.buildHtml().remoteDialog), b.modalobj.find(".btn-primary").click(function () {
                ...
                c.length > 0 && "image" == b.options.type && a.getJSON("/upload/fetch.html?type=image&module="+module, {url: c}, function (a) {
            ...
            function c(a) {
                ...
                } else if (a.indexOf("v.youku.com") >= 0)b = a.match(/id_(.*)\.html/), c = "http://player.youku.com/embed/" + b[1]; else {
                    if (!(a.indexOf("tudou.com") >= 0))return;
                    b = a.match(/\/([-\w]+)/g), b = b[b.length - 1].substring(1), c = "http://www.tudou.com/programs/view/html5embed.action?code=" + b
                }
                return c
            }
            ...
            var e = this;
            e.modalobj.find("#li_network").removeClass("hide"), e.modalobj.find(".modal-body").append(e.buildHtml().remoteVideoDialog), e.modalobj.find("#networkurl").blur(function () {
                var c = a(this).val();
                ...
                if (a.length > 0 && "video" == e.options.type) {
                    var b = c(a);
                    b = d(b), e.finish([{url: b, isRemote: !0}])
                }
            })
        }, initLocal: function () {
            var a = this;
            a.modalobj.find("#li_history_image").removeClass("hide"), a.modalobj.find(".modal-body").append(this.buildHtml().localDialog), a.localPage(1)
        }, localPage: function (c) {
            var d = this;
            if (d.options.isWechat)var e = d.options.type, f = d.options.mode, g = "./index.php?c=utility&a=wechat_file&do=browser", h = {
                page: c,
                type: e,
                mode: f,
                psize: 32
            ...
            var e = d.options.type;
            var i = d.modalobj.find("#select-year .btn-info").data("id"), j = d.modalobj.find("#select-month .btn-info").data("id"), g = "/upload/local.html?module="+module, h = {
                page: c,
                year: i,
                type: e,
                month: j
            ...
            return a.getJSON(g, h, function (c) {
                c = c.message, k.find(".history-content").css("text-align", "center").html('<i class="fa fa-spinner fa-pulse fa-5x"></i>'), k.find("#image-list-pager").html(""), b.isEmpty(c.items) ? k.find(".history-content").css("text-align", "left").html('<i class="fa fa-info-circle"></i> 暂无数据') : (k.data("attachment", c.items), k.find(".history-content").empty(), k.find(".history-content").html(b.template(d.buildHtml()[d.options.isWechat ? "weixin_localDialogLi" : "localDialogLi"])(c)), k.find("#image-list-pager").html(c.page), k.find(".pagination a").click(function () {
            ...
            b.modalobj.find("#history_image .img-list li .btnClose").unbind().click(function () {
                ...
                return c ? (a.post("/upload/delete.html", {id: c}, function (a) {
            ...
            b.modalobj.find(".history .delete-file").off("click"), b.modalobj.find(".history .delete-file").on("click", function (b) {
                var c = a(this);
                if (confirm("确定要删除文件吗？")) {
                    var d = a(this).parent().attr("attachid"), e = a(this).parent().attr("data-type");
                    a.post("./index.php?c=utility&a=wechat_file&do=delete", {id: d}, function (b) {
                        var b = a.parseJSON(b);
            ...
            var c = this;
            a(b).toggleClass("img-item-selected"), c.options.isWechat ? c.options.direct && c.modalobj.find("#history_image").find(".btn-primary").trigger("click") : c.options.multiple || c.modalobj.find("#history_image").find(".btn-primary").trigger("click")
        }, initLocalAudio: function () {
            var a = this;
            a.modalobj.find("#li_history_audio").removeClass("hide"), a.modalobj.find(".modal-body").append(this.buildHtml().localAudioDialog), a.localAudioPage(1)
            ...
            if (d.options.isWechat)var e = d.options.type, f = d.options.mode, g = "./index.php?c=utility&a=wechat_file&do=browser", h = {
                ...
                module = $("#module").val();
            var g = "/upload/local.html?c=utility&a=file&do=local&type=audio&pagesize=5&module="+module, h = {page: c};
            var i = d.modalobj.find("#history_audio");
            return a.getJSON(g, h, function (c) {
                c = c.message, i.find(".history-content").html('<i class="fa fa-spinner fa-pulse"></i>'), b.isEmpty(c.items) ? i.find(".history-content").css("text-align", "center").html('<i class="fa fa-info-circle"></i> 暂无数据') : (i.data("attachment", c.items), i.find(".history-content").empty(), i.find(".history-content").html(b.template(d.buildHtml()[d.options.isWechat ? "weixin_localAudioDialogLi" : "localAudioDialogLi"])(c)), i.find("#image-list-pager").html(c.page), i.find(".pagination a").click(function () {
                    d.localAudioPage(a(this).attr("page"))
                }), i.find(".js-btn-select").click(function (b) {
                    a(b.target).toggleClass("btn-primary"), d.options.isWechat ? d.options.direct && d.modalobj.find("#history_audio").find(".modal-footer .btn-primary").trigger("click") : d.options.multiple || d.modalobj.find("#history_audio").find(".modal-footer .btn-primary").trigger("click")
            ...
            var a = this;
            a.modalobj.find("#li_history_video").removeClass("hide"), a.modalobj.find(".modal-body").append(this.buildHtml().localVideoDialog), a.localVideoPage(1)
        }, localVideoPage: function (c) {
            var d = this;
            if (d.options.isWechat)var e = d.options.type, f = "./index.php?c=utility&a=wechat_file&do=browser", g = {
                page: c,
                type: e,
                psize: 5
            }; else
                module = $("#module").val();
            var f = "/upload/local.html?module="+module+"&a=file&do=local&type=video&pagesize=5", g = {page: c};
            var h = d.modalobj.find("#history_video");
            return a.getJSON(f, g, function (c) {
                c = c.message, h.find(".history-content").html('<i class="fa fa-spinner fa-pulse"></i>'), b.isEmpty(c.items) ? h.find(".history-content").css("text-align", "left").html('<i class="fa fa-info-circle"></i> 暂无数据') : (h.data("attachment", c.items), h.find(".history-content").empty(), h.find(".history-content").html(b.template(d.buildHtml()[d.options.isWechat ? "weixin_localVideoDialogLi" : "localVideoDialogLi"])(c)), h.find("#image-list-pager").html(c.page), h.find(".pagination a").click(function () {
                    d.localVideoPage(a(this).attr("page"))
                }), h.find(".js-btn-select").click(function (b) {
                    a(b.target).toggleClass("btn-primary"), d.options.isWechat ? d.options.direct && d.modalobj.find("#history_video").find(".modal-footer .btn-primary").trigger("click") : d.options.multiple || d.modalobj.find("#history_video").find(".modal-footer .btn-primary").trigger("click")
                }), d.options.isWechat && d.weixinDeletefile())
            }), h.find(".modal-footer .btn-primary").unbind("click").click(function () {
                var b = [];
                h.find(".history-content .btn-primary").each(function () {
                    b.push(d.modalobj.find("#history_video").data("attachment")[a(this).attr("attachid")]), a(this).removeClass("btn-primary")
                }), d.finish(b)
            }), !1
        }, finish: function (b) {
            var c = this;
